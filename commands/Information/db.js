...
module.exports = class NCommand extends Command {
    ...
    async run(message, { user }) {
      try {  let { body } = await superagent
        ...
        .addField(`Library`, body.lib ? body.lib : "None", true)
        .addField(`Tags`, body.tags ? body.tags : "None", true)
        .addField(`Owner(s)`, `${body.owners.map(owner => `<@${owner}>`).join('\n')}`, true)
        .addField(`Github`, `${body.github ? `[Click Here](${body.github})` : "N/A"}`, true)
        .addField(`Website`, `${body.website ? `[Click Here](${body.website})` : "N/A"}`, true)
